class_name ThrowingChaseBehavior extends BehaviorState
...
func do_behavior(_delta):
	...
	var targetPlayer:ThrowingPlayer = attackTarget.playerController as ThrowingPlayer ## Get throwing player host that we are attacking
	var targetPlayerShield:HostController = targetPlayer.grabbedHost
	
	var canSeeTarget:bool = host.has_LOS_to_host(attackTarget, attackDistance)
	var canSeeTargetShield:bool = is_instance_valid(targetPlayerShield) and host.has_LOS_to_host(targetPlayerShield, attackDistance)

	var targetInRange:bool = _dist_to_target() <= attackDistance
	
	

	if (!canSeeTarget and !canSeeTargetShield) or !targetInRange:
		_reset_attack_delay()
		enemy.path_to_host(attackTarget)
	
	elif canSeeTarget or canSeeTargetShield:
